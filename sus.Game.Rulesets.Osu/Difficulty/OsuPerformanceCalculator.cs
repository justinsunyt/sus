...
using sus.Game.Rulesets.Osu.Difficulty.Skills;
using sus.Game.Rulesets.Osu.Mods;
using sus.Game.Rulesets.Osu.Scoring;
using sus.Game.Rulesets.Scoring;
using sus.Game.Scoring;

namespace sus.Game.Rulesets.Osu.Difficulty
{
    public class OsuPerformanceCalculator : PerformanceCalculator
    {
        ...
        {
            var susAttributes = (OsuDifficultyAttributes)attributes;
            ...
            countSliderEndsDropped = susAttributes.SliderCount - score.Statistics.GetValueOrDefault(HitResult.SliderTailHit);
            ...
            if (susAttributes.SliderCount > 0)
            {
                if (usingClassicSliderAccuracy)
                {
                    // Consider that full combo is maximum combo minus dropped slider tails since they don't contribute to combo but also don't break it
                    // In classic scores we can't know the amount of dropped sliders so we estimate to 10% of all sliders on the map
                    double fullComboThreshold = attributes.MaxCombo - 0.1 * susAttributes.SliderCount;
            ...
            if (score.Mods.Any(m => m is OsuModSpunOut) && totalHits > 0)
                multiplier *= 1.0 - Math.Pow((double)susAttributes.SpinnerCount / totalHits, 0.85);
            ...
            speedDeviation = calculateSpeedDeviation(susAttributes);

            double aimValue = computeAimValue(score, susAttributes);
            double speedValue = computeSpeedValue(score, susAttributes);
            double accuracyValue = computeAccuracyValue(score, susAttributes);
            double flashlightValue = computeFlashlightValue(score, susAttributes);

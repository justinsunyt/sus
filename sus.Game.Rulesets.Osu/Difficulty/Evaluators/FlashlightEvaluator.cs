...
namespace sus.Game.Rulesets.Osu.Difficulty.Evaluators
{
    public static class FlashlightEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(DifficultyHitObject current, bool hidden)
        {
            ...
            var susCurrent = (OsuDifficultyHitObject)current;
            var susHitObject = (OsuHitObject)(susCurrent.BaseObject);

            double scalingFactor = 52.0 / susHitObject.Radius;
            double smallDistNerf = 1.0;
            double cumulativeStrainTime = 0.0;

            double result = 0.0;

            OsuDifficultyHitObject lastObj = susCurrent;
            ...
            for (int i = 0; i < Math.Min(current.Index, 10); i++)
            {
                ...
                {
                    double jumpDistance = (susHitObject.StackedPosition - currentHitObject.StackedEndPosition).Length;
                    ...
                    double opacityBonus = 1.0 + max_opacity_bonus * (1.0 - susCurrent.OpacityAt(currentHitObject.StartTime, hidden));

                    result += stackNerf * opacityBonus * scalingFactor * jumpDistance / cumulativeStrainTime;

                    if (currentObj.Angle != null && susCurrent.Angle != null)
                    {
                        // Objects further back in time should count less for the nerf.
                        if (Math.Abs(currentObj.Angle.Value - susCurrent.Angle.Value) < 0.02)
            ...
            if (susCurrent.BaseObject is Slider susSlider)
            {
                // Invert the scaling factor to determine the true travel distance independent of circle size.
                double pixelTravelDistance = susSlider.LazyTravelDistance / scalingFactor;

                // Reward sliders based on velocity.
                sliderBonus = Math.Pow(Math.Max(0.0, pixelTravelDistance / susCurrent.TravelTime - min_velocity), 0.5);

                // Longer sliders require more memorisation.
                sliderBonus *= pixelTravelDistance;

                // Nerf sliders with repeats, as less memorisation is required.
                if (susSlider.RepeatCount > 0)
                    sliderBonus /= (susSlider.RepeatCount + 1);
            }

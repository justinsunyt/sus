...
namespace sus.Game.Rulesets.Osu.Difficulty.Evaluators
{
    public static class SpeedEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(DifficultyHitObject current, IReadOnlyList<Mod> mods)
        {
            ...
            var susPrevObj = current.Index > 0 ? (OsuDifficultyHitObject)current.Previous(0) : null;

            double strainTime = susCurrObj.StrainTime;
            double doubletapness = 1.0 - susCurrObj.GetDoubletapness((OsuDifficultyHitObject?)susCurrObj.Next(0));

            // Cap deltatime to the OD 300 hitwindow.
            // 0.93 is derived from making sure 260bpm OD8 streams aren't nerfed harshly, whilst 0.92 limits the effect of the cap.
            strainTime /= Math.Clamp((strainTime / susCurrObj.HitWindowGreat) / 0.93, 0.92, 1);
            ...
            double travelDistance = susPrevObj?.TravelDistance ?? 0;
            double distance = travelDistance + susCurrObj.MinimumJumpDistance;

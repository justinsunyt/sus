...
namespace sus.Game.Rulesets.Osu.Mods
{
                                        ...
                                        IApplicableToHealthProcessor, IApplicableToDifficulty, IApplicableFailOverride, IHasSeed, IHidesApproachCircles
    {
        ...
        public override void ApplyToBeatmap(IBeatmap beatmap)
        {
            ...
            controlPointInfo = susBeatmap.ControlPointInfo;

            var originalHitObjects = susBeatmap.HitObjects.OrderBy(x => x.StartTime).ToList();
            var hitObjects = generateBeats(susBeatmap, originalHitObjects)
                             .Select(beat =>
                             {
                                 var newCircle = new HitCircle();
                                 newCircle.ApplyDefaults(controlPointInfo, susBeatmap.Difficulty);

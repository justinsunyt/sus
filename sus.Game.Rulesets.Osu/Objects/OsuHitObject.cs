    ...
    public abstract class OsuHitObject : HitObject, IHasComboInformation, IHasPosition, IHasTimePreempt
    {
        ...
        protected OsuHitObject()
        {
            StackHeightBindable.BindValueChanged(height =>
            {
                ...
                {
                    if (nested is OsuHitObject susHitObject)
                        susHitObject.StackHeight = height.NewValue;
                }
            });
        }
        ...
        {
            // Note that this implementation is shared with the sus!catch ruleset's implementation.

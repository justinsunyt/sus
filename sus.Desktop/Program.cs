...
namespace sus.Desktop
{
    ...
    {
#if DEBUG
        private const string base_game_name = @"sus-development";
#else
        private const string base_game_name = @"sus";
#endif
        ...
        public static void Main(string[] args)
        {
                ...
                if (windowsVersion.Major < 6 || (windowsVersion.Major == 6 && windowsVersion.Minor <= 2))
                {
                    unsafe
                    {
                        ...
                        SDL3.SDL_ShowSimpleMessageBox(SDL_MessageBoxFlags.SDL_MESSAGEBOX_ERROR,
                            "Your operating system is too old to run sus!"u8,
                            "This version of sus! requires at least Windows 8.1 to run.\n"u8
                            + "Please upgrade your operating system or consider using an older version of sus!.\n\n"u8
                            + "If you are running a newer version of windows, please check you don't have \"Compatibility mode\" turned on for sus!"u8, null);
                        return;
                    }
                }
            }
            ...
            using (DesktopGameHost host = Host.GetSuitableDesktopHost(gameName, hostOptions))
            {
                if (!host.IsPrimaryInstance)
                {
                    ...
                    if (!DebugUtils.IsDebugBuild)
                    {
                        Logger.Log(@"sus! does not support multiple running instances.", LoggingTarget.Runtime, LogLevel.Error);
                        return;
                    }
                }
        ...
        private static bool trySendIPCMessage(IIpcHost host, string cwd, string[] args)
        {
            ...
            {
                var susSchemeLinkHandler = new OsuSchemeLinkIPCChannel(host);
                if (!susSchemeLinkHandler.HandleLinkAsync(args[0]).Wait(3000))
                    throw new IPCTimeoutException(susSchemeLinkHandler.GetType());

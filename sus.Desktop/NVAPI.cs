...
namespace sus.Desktop
{
    ...
    {
        private const string sus_filename = "sus!.exe";
        ...
        public const string APPLICATION_NAME = @"sus!";
        public const string PROFILE_NAME = @"sus!";
        ...
        private static bool containsApplication(IntPtr profileHandle, NvProfile profile, out NvApplication application)
        {
            ...
            {
                if (applications[i].AppName == sus_filename)
                {
        ...
        private static bool getProfile(out IntPtr profileHandle, out NvApplication application, out bool isApplicationSpecific)
        {
            ...
            if (checkError(FindApplicationByName(sessionHandle, sus_filename, out profileHandle, ref application), nameof(FindApplicationByName)))
            {
        ...
        private static bool setSetting(NvSettingID settingId, uint settingValue)
        {
            ...
            {
                // We don't want to interfere with the user's other settings, so let's create a separate config for sus!
            ...
            if (!containsApplication(profileHandle, profile, out application))
            {
                ...
                application.AppName = sus_filename;

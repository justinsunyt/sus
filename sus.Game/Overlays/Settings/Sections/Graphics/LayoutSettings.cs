...
using sus.Framework.Platform.Windows;
...
namespace sus.Game.Overlays.Settings.Sections.Graphics
{
    public partial class LayoutSettings : SettingsSubsection
    {
        ...
        private void load(FrameworkConfigManager config, OsuConfigManager susConfig, GameHost host)
        {
            window = host.Window;

            scalingMode = susConfig.GetBindable<ScalingMode>(OsuSetting.Scaling);
            sizeFullscreen = config.GetBindable<Size>(FrameworkSetting.SizeFullscreen);
            scalingSizeX = susConfig.GetBindable<float>(OsuSetting.ScalingSizeX);
            scalingSizeY = susConfig.GetBindable<float>(OsuSetting.ScalingSizeY);
            scalingPositionX = susConfig.GetBindable<float>(OsuSetting.ScalingPositionX);
            scalingPositionY = susConfig.GetBindable<float>(OsuSetting.ScalingPositionY);
            scalingBackgroundDim = susConfig.GetBindable<float>(OsuSetting.ScalingBackgroundDim);
            ...
            Children = new Drawable[]
            {
                ...
                safeAreaConsiderationsCheckbox = new SettingsCheckbox
                {
                    ...
                    Current = susConfig.GetBindable<bool>(OsuSetting.SafeAreaConsiderations),
                },
                new SettingsSlider<float, UIScaleSlider>
                {
                    LabelText = GraphicsSettingsStrings.UIScaling,
                    TransferValueOnCommit = true,
                    Current = susConfig.GetBindable<float>(OsuSetting.UIScale),
                    KeyboardStep = 0.01f,
                ...
                new SettingsEnumDropdown<ScalingMode>
                {
                    ...
                    Current = susConfig.GetBindable<ScalingMode>(OsuSetting.Scaling),
                    Keywords = new[] { "scale", "letterbox" },
                },
        ...
        /// <summary>
        /// Contrary to <see cref="Display.Equals(sus.Framework.Platform.Display?)"/>, this comparer disregards the value of <see cref="Display.Bounds"/>.

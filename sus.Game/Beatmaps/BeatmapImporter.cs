...
namespace sus.Game.Beatmaps
{
    ...
    public class BeatmapImporter : RealmArchiveModelImporter<BeatmapSetInfo>
    {
        ...
        protected override string[] HashableFileTypes => new[] { ".sus" };
        ...
        public override async Task<Live<BeatmapSetInfo>?> ImportAsUpdate(ProgressNotification notification, ImportTask importTask, BeatmapSetInfo original)
        {
            ...
            first.PerformWrite(updated =>
            {
                try
                {
                    ...
                    // but that isn't a guarantee (ie. if the .sus file doesn't have OnlineIDs populated).
        ...
        {
            // let's make sure there are actually .sus files to import.
            string? mapName = reader.Filenames.FirstOrDefault(f => f.EndsWith(".sus", StringComparison.OrdinalIgnoreCase));
            ...
            using (var stream = new LineBufferedReader(reader.GetStream(mapName)))
            {
                ...
                {
                    Logger.Log($"No content found in first .sus file of beatmap archive ({reader.Name} / {mapName})", LoggingTarget.Database);
                    return null;
                }
        ...
        /// For legacy imports, we can use the oldest file write time for any `.sus` file in the directory.
        ...
        private DateTimeOffset getDateAdded(ArchiveReader? reader)
        {
            ...
            {
                var beatmaps = reader.Filenames.Where(f => f.EndsWith(".sus", StringComparison.OrdinalIgnoreCase));
        ...
        private List<BeatmapInfo> createBeatmapDifficulties(BeatmapSetInfo beatmapSet, Realm realm)
        {
            ...
            foreach (var file in beatmapSet.Files.Where(f => f.Filename.EndsWith(".sus", StringComparison.OrdinalIgnoreCase)))
            {

...
namespace sus.Game.Beatmaps.Formats
{
    public abstract class LegacyDecoder<T> : Decoder<T>
        where T : new()
    {
        // If this is updated, a new release of `sus-server-beatmap-submission` is required with updated packages.
        // See usage at https://github.com/ppy/sus-server-beatmap-submission/blob/master/sus.Server.BeatmapSubmission/Services/BeatmapPackageParser.cs#L96-L97.
        public const int LATEST_VERSION = 14;

        public const int MAX_COMBO_COLOUR_COUNT = 8;

        /// <summary>
        /// The .sus format (beatmap) version.
        ///
        /// sus!stable's versions end at <see cref="LATEST_VERSION"/>.
        /// sus!lazer's versions starts at <see cref="LegacyBeatmapEncoder.FIRST_LAZER_VERSION"/>.
        /// </summary>
        ...
        protected string CleanFilename(string path) => path
                                                       // User error which is supported by stable (https://github.com/ppy/sus/issues/21204)
                                                       .Replace(@"\\", @"\")
                                                       .Trim('"')
                                                       .ToStandardisedPath();

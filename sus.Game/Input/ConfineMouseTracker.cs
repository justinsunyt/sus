...
namespace sus.Game.Input
{
    ...
    public partial class ConfineMouseTracker : Component
    {
        ...
        private Bindable<OsuConfineMouseMode> susConfineMode;
        private IBindable<LocalUserPlayingState> localUserPlaying;

        [BackgroundDependencyLoader]
        private void load(ILocalUserPlayInfo localUserInfo, FrameworkConfigManager frameworkConfigManager, OsuConfigManager susConfigManager)
        {
            ...
            susConfineMode = susConfigManager.GetBindable<OsuConfineMouseMode>(OsuSetting.ConfineMouseMode);
            localUserPlaying = localUserInfo.PlayingState.GetBoundCopy();

            susConfineMode.ValueChanged += _ => updateConfineMode();

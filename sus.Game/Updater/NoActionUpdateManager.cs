    ...
    public partial class NoActionUpdateManager : UpdateManager
    {
        ...
        protected override async Task<bool> PerformUpdateCheck(CancellationToken cancellationToken)
        {
            try
            {
                ...
                OsuJsonWebRequest<GitHubRelease[]> releasesRequest = new OsuJsonWebRequest<GitHubRelease[]>("https://api.github.com/repos/ppy/sus/releases?per_page=10&page=1");
                ...
                if (latestTagName != version)
                {
                    ...
                    {
                        Text = $"A newer release of sus! has been found ({version} → {latestTagName}).\n\n"
                               + "Check with your package manager / provider to bring sus! up-to-date!",
                    });

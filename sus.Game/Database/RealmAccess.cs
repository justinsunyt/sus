...
namespace sus.Game.Database
{
    ...
    public class RealmAccess : IDisposable
    {
        ...
        /// 36   2023-10-26    Add LegacyOnlineID to ScoreInfo. Move sus_scores_*_high IDs stored in OnlineID to LegacyOnlineID. Reset anomalous OnlineIDs.
        ...
        private void attemptRecoverFromFile(string recoveryFilename)
        {
            ...
            try
            {
                using (var realm = Realm.GetInstance(getConfiguration()))
                {
                    if (realm.All<ScoreInfo>().Any())
                    {
                        ...
                        Logger.Log($@"To perform recovery, delete {OsuGameBase.CLIENT_DATABASE_FILENAME} while sus! is not running.", LoggingTarget.Database);
                        return;
                    }
                }
            }
            ...
            {
                Logger.Log(@"Recovery aborted as the newer version could not be loaded by this sus! version.", LoggingTarget.Database);
                return;
            }
        ...
        private Realm prepareFirstRealmAccess()
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    Logger.Error(e, "Your local database is too new to work with this version of sus!. Please close sus! and install the latest release to recover your data.");
        ...
        public Task WriteAsync(Action<Realm> action)
        {
            ...
            var writeTask = Task.Run(async () =>
            {
                ...
                using (var realm = getRealmInstance())
                    // ReSharper disable once AccessToDisposedClsusre (WriteAsync should be marked as [InstantHandle]).
        ...
        /// </summary>
        /// <remarks>
        /// This adds sus! specific thread and managed state safety checks on top of <see cref="IRealmCollection{T}.SubscribeForNotifications"/>.
        ///
        ...
        {
            // This is currently the only usage of temporary files at the sus! side.
        ...
        private void applyMigrationsForVersion(Migration migration, ulong targetVersion)
        {
            ...
            switch (targetVersion)
            {
                ...
                case 42:
                    ...
                    {
                        // https://github.com/ppy/sus/blob/8773c2f7ebc226942d6124eb95c07a83934272ea/sus.Game.Rulesets.Mania/ManiaRuleset.cs#L327-L336
                        ...
                        // https://github.com/ppy/sus/blob/8773c2f7ebc226942d6124eb95c07a83934272ea/sus.Game.Rulesets.Mania/ManiaInputManager.cs#L22-L31

...
{
    /// <summary>
    /// A class which handles importing legacy user data of a single type from sus-stable.
    /// </summary>
    public abstract class LegacyModelImporter<TModel>
        where TModel : class, IHasGuidPrimaryKey
    {
        /// <summary>
        /// The relative path from sus-stable's data directory to import items from.
        /// </summary>
        ...
        public Task ImportFromStableAsync(StableStorage stableStorage)
        {
            ...
            if (!storage.ExistsDirectory(ImportFromStablePath))
            {
                ...
                Logger.Log(@$"Folder ""{fullPath}"" not available in the target sus!stable installation to import {Importer.HumanisedModelName}s.", LoggingTarget.Information, LogLevel.Error);

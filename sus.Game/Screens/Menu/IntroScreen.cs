...
namespace sus.Game.Screens.Menu
{
    public abstract partial class IntroScreen : StartupScreen
    {
        ...
        /// <summary>
        /// Whether the <see cref="Track"/> is provided by sus! resources, rather than a user beatmap.
        ...
        private void load(OsuConfigManager config, Framework.Game game, RealmAccess realm, IAPIProvider api)
        {
            ...
            {
                // Intro beatmaps are generally made using the sus! ruleset.
                // It might not be present in test projects for other rulesets.
                bool susRulesetPresent = rulesets.GetRuleset(0) != null;

                if (!loadThemedIntro() && susRulesetPresent)
                {
        ...
        protected override void LogoArriving(OsuLogo logo, bool resuming)
        {
            ...
            if (!resuming)
            {
                ...
                // an exception is running ruleset tests, where the sus! ruleset may not be present (causing importing the intro to fail).
                if (initialBeatmap != null)

...
namespace sus.Game.Tournament.IPC
{
    public partial class FileBasedIPC : MatchIPCInfo
    {
        ...
        private string? findFromLocalAppData()
        {
            ...
            string stableInstallPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), @"sus!");
        ...
        private string? findFromDotFolder()
        {
            ...
            string stableInstallPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".sus");
...
#pragma warning disable CA1416
                using (RegistryKey? key = Registry.ClassesRoot.OpenSubKey("sus"))
                    stableInstallPath = key?.OpenSubKey(@"shell\open\command")?.GetValue(string.Empty)?.ToString()?.Split('"')[1].Replace("sus!.exe", "");

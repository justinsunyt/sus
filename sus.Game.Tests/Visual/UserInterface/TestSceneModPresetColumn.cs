...
namespace sus.Game.Tests.Visual.UserInterface
{
    public partial class TestSceneModPresetColumn : OsuManualInputManagerTestScene
    {
        ...
        {
            AddStep("set sus! ruleset", () => Ruleset.Value = rulesets.GetRuleset(0));
            ...
            AddStep("add another sus! preset", () => Realm.Write(r => r.Add(new ModPreset
            {
                Name = "hdhr",
                Mods = new Mod[]
                {
                ...
                Ruleset = r.Find<RulesetInfo>("sus")!
            })));
            AddUntilStep("2 panels visible", () => this.ChildrenOfType<ModPresetPanel>().Count() == 2);

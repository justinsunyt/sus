...
namespace sus.Game.Tests.Visual.UserInterface
{
    ...
    public partial class TestSceneModSelectOverlay : OsuManualInputManagerTestScene
    {
        ...
        {
            var susModDoubleTime = new OsuModDoubleTime { SpeedChange = { Value = 1.2 } };

            createScreen();
            changeRuleset(0);

            AddStep("set dt mod with custom rate", () => { SelectedMods.Value = new[] { susModDoubleTime }; });
        ...
        public void TestKeepSharedSettingsFromSimilarMods()
        {
            ...
            {
                var susMod = getSelectedMod<OsuModDifficultyAdjust>();

                susMod.ExtendedLimits.Value = true;
                susMod.CircleSize.Value = setting_change;
                susMod.DrainRate.Value = setting_change;
                susMod.OverallDifficulty.Value = setting_change;
                susMod.ApproachRate.Value = setting_change;
            });

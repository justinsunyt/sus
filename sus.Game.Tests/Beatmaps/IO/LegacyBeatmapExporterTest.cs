...
namespace sus.Game.Tests.Beatmaps.IO
{
    ...
    public partial class LegacyBeatmapExporterTest : OsuTestScene
    {
        ...
        public void TestExportStability()
        {
            ...
            const string sus_filename = @"legacy export - stability test (spaceman_atlas) [].sus";

            AddAssert("exports are identical",
                () => getStringContentsOf(sus_filename, firstExport.GetBuffer()),
                () => Is.EqualTo(getStringContentsOf(sus_filename, secondExport.GetBuffer())));

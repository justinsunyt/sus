        ...
        public void TestMigration()
        {
            ...
            using (var host = new CustomTestHeadlessGameHost())
            {
                try
                {
                    ...
                    var sus = LoadOsuIntoHost(host);
                    var storage = sus.Dependencies.Get<Storage>();
                    var susStorage = storage as MigratableStorage;
                    ...
                    Assert.That(susStorage, Is.Not.Null);
                    ...
                    foreach (string file in susStorage.IgnoreFiles)
                    {
                    ...
                    foreach (string dir in susStorage.IgnoreDirectories)
                    {
        ...
        public void TestMigrationBetweenTwoTargets()
        {
            ...
            using (var host = new CustomTestHeadlessGameHost())
            {
                try
                {
                    ...
                    Assert.DoesNotThrow(() => sus.Migrate(customPath));
                    Assert.That(File.Exists(Path.Combine(customPath, OsuGameBase.CLIENT_DATABASE_FILENAME)));

                    Assert.DoesNotThrow(() => sus.Migrate(customPath2));
        ...
        public void TestMigrationToSameTargetFails()
        {
            ...
            using (var host = new CustomTestHeadlessGameHost())
            {
                try
                {
                    ...
                    Assert.DoesNotThrow(() => sus.Migrate(customPath));
                    Assert.Throws<ArgumentException>(() => sus.Migrate(customPath));
                }
                finally
                {
                    host.Exit();
        ...
        public void TestMigrationFailsOnExistingData()
        {
            ...
            using (var host = new CustomTestHeadlessGameHost())
            {
                try
                {
                    ...
                    var storage = sus.Dependencies.Get<Storage>();
                    var susStorage = storage as OsuStorage;
                    ...
                    Assert.Throws<ArgumentException>(() => sus.Migrate(customPath2));

                    susStorage?.ChangeDataPath(customPath2);

                    Assert.That(susStorage?.CustomStoragePath, Is.EqualTo(customPath2));
        ...
        public void TestMigrationToNestedTargetFails()
        {
            ...
            using (var host = new CustomTestHeadlessGameHost())
            {
                try
                {
                    ...
                    Assert.Throws<ArgumentException>(() => sus.Migrate(subFolder));
                }
                finally
                {
                    host.Exit();
        ...
        public void TestMigrationToSeeminglyNestedTarget()
        {
            ...
            using (var host = new CustomTestHeadlessGameHost())
            {
                try
                {
                    ...
                    sus.Migrate(seeminglySubFolder);
                }
                finally
                {
                    host.Exit();

...
namespace sus.Game.Tests
{
    public abstract partial class ImportTest
    {
        ...
        {
            var sus = new TestOsuGameBase(withBeatmap);
            Task.Factory.StartNew(() => host.Run(sus), TaskCreationOptions.LongRunning)
                .ContinueWith(t => Assert.Fail($"Host threw exception {t.Exception}"), TaskContinuationOptions.OnlyOnFaulted);

            waitForOrAssert(() => sus.IsLoaded, @"sus! failed to start in a reasonable amount of time");
            ...
            waitForOrAssert(() => ready, @"sus! failed to start in a reasonable amount of time");

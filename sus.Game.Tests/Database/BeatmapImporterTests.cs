        ...
        public void TestImportDirectoryWithEmptyOsuFiles()
        {
            RunTestWithRealmAsync(async (realm, storage) =>
            {
                ...
                try
                {
                    ...
                    foreach (var file in new DirectoryInfo(extractedFolder).GetFiles("*.sus"))
                    {
        ...
        public void TestImportThenImportWithChangedHashedFile()
        {
            RunTestWithRealmAsync(async (realm, storage) =>
            {
                ...
                try
                {
                    ...
                    using (var sw = new FileInfo(Directory.GetFiles(extractedFolder, "*.sus").First()).AppendText())
        ...
        public void TestRollbackOnFailure()
        {
            RunTestWithRealmAsync(async (realm, storage) =>
            {
                ...
                using (var zip = ZipArchive.Open(brokenOsz))
                {
                    ...
                    {
                        if (entry.Key!.EndsWith(".sus", StringComparison.InvariantCulture))
                            zip.RemoveEntry(entry);
                    }

                    zip.AddEntry("broken.sus", brokenOsu, false);
                    zip.SaveTo(outStream, CompressionType.Deflate);
                }

                // this will trigger purging of the existing beatmap (online set id match) but should rollback due to broken sus.
                try
                {
        ...
        public void TestImportThenReimportWithNewDifficulty()
        {
            RunTestWithRealmAsync(async (realm, storage) =>
            {
                ...
                try
                {
                    ...
                    new FileInfo(Directory.GetFiles(extractedFolder, "*.sus").First()).Delete();
                    ...
                    // Second import matches first but contains one extra .sus file.
        ...
        public void TestImportWithDuplicateHashes()
        {
            RunTestWithRealmAsync(async (realm, storage) =>
            {
                ...
                try
                {
                    ...
                    {
                        zip.AddAllFromDirectory(extractedFolder);
                        zip.AddEntry("duplicate.sus", Directory.GetFiles(extractedFolder, "*.sus").First());
                        zip.SaveTo(temp, new ZipWriterOptions(CompressionType.Deflate));
                    }

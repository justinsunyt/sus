...
namespace sus.Game.Tests.Database
{
    ...
    public class BeatmapImporterUpdateTests : RealmTest
    {
        ...
        public void TestNewDifficultyAdded()
        {
            RunTestWithRealmAsync(async (realm, storage) =>
            {
                ...
                using var _ = getBeatmapArchiveWithModifications(out string pathMissingOneBeatmap, directory =>
                {
                    ...
                    directory.GetFiles("*.sus").First().Delete();
                });
        ...
        /// <summary>
        /// Regression test covering https://github.com/ppy/sus/issues/19369 (import potentially duplicating if original has no <see cref="BeatmapInfo.OnlineID"/>).
        /// </summary>
        [Test]
        ...
        public void TestExistingDifficultyModified()
        {
            RunTestWithRealmAsync(async (realm, storage) =>
            {
                ...
                {
                    // Modify one .sus file with different content.
                    var firstOsuFile = directory.GetFiles("*.sus").First();
        ...
        public void TestScoreTransferredOnUnchanged()
        {
            RunTestWithRealmAsync(async (realm, storage) =>
            {
                ...
                using var _ = getBeatmapArchiveWithModifications(out string pathMissingOneBeatmap, directory =>
                {
                    ...
                    var fileToRemove = directory.GetFiles("*.sus").First();
        ...
        public void TestCollectionTransferModifiedBeatmap()
        {
            RunTestWithRealmAsync(async (realm, storage) =>
            {
                ...
                {
                    // Modify one .sus file with different content.
                    var firstOsuFile = directory.GetFiles("*[Hard]*.sus").First();
                ...
                // Second import matches first but contains a modified .sus file.
                var importAfterUpdate = await importer.ImportAsUpdate(new ProgressNotification(), new ImportTask(pathModified), importBeforeUpdate.Value);

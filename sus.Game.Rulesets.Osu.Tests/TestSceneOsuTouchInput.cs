    ...
    public partial class TestSceneOsuTouchInput : OsuManualInputManagerTestScene
    {
        ...
        public void SetUpSteps()
        {
            ...
            AddStep("Create tests", () =>
            {
                ...
                {
                    susInputManager = new OsuInputManager(new OsuRuleset().RulesetInfo)
                    {
        ...
        public void TestMovementWhileDisallowed()
        {
            ...
            AddStep("Disallow gameplay cursor movement", () => susInputManager.AllowUserCursorMovement = false);

            Vector2? positionBefore = null;

            AddStep("Store cursor position", () => positionBefore = susInputManager.CurrentState.Mouse.Position);
            beginTouch(TouchSource.Touch1);

            assertKeyCounter(1, 0);
            checkPressed(OsuAction.LeftButton);
            AddAssert("Cursor position unchanged", () => susInputManager.CurrentState.Mouse.Position, () => Is.EqualTo(positionBefore));
        }
        ...
        public void TestActionWhileDisallowed()
        {
            ...
            AddStep("Disallow gameplay actions", () => susInputManager.AllowGameplayInputs = false);
        ...
        {
            return new Vector2(
                susInputManager.ScreenSpaceDrawQuad.Centre.X + susInputManager.ScreenSpaceDrawQuad.Width * (-1 + (int)source) / 8,
                susInputManager.ScreenSpaceDrawQuad.Centre.Y - 100
            );
        }

        private void checkPosition(TouchSource touchSource) =>
            AddAssert("Cursor position is correct", () => susInputManager.CurrentState.Mouse.Position, () => Is.EqualTo(getSanePositionForSource(touchSource)));
        ...
        private void releaseAllTouches()
        {
            AddStep("Release all touches", () =>
            {
                ...
                foreach (TouchSource source in InputManager.CurrentState.Touch.ActiveSources)
                    InputManager.EndTouch(new Touch(source, susInputManager.ScreenSpaceDrawQuad.Centre));
            });
        }

        private void checkNotPressed(OsuAction action) => AddAssert($"Not pressing {action}", () => !susInputManager.PressedActions.Contains(action));
        private void checkPressed(OsuAction action) => AddAssert($"Is pressing {action}", () => susInputManager.PressedActions.Contains(action));
